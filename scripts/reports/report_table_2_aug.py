...
def main():
    ...
    for parent, target_hf, target_name in TABLE_2_MODELS:
        ...
        if os.path.exists(nm_result_file):
            with open(nm_result_file) as f:
                nm_data = json.load(f)
            nm_aug_res = _extract_score_test_metrics(nm_data, "aug_min_k_20_logprob_mean")
            ...
            if "shuffled_label_control" in data and isinstance(data["shuffled_label_control"], list):
                for ctrl in data["shuffled_label_control"]:
                    if ctrl.get("score_name") == "aug_min_k_20_logprob_mean":
        ...
        if os.path.exists(result_file):
            ...
            aug_res = _extract_score_test_metrics(data, "aug_min_k_20_logprob_mean")
    ...
    with open("outputs/replicated_table_2_aug.txt", "w") as f:
        f.write(output_str)
    print("\nSaved compiled table to outputs/replicated_table_2_aug.txt")
