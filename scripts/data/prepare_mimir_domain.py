...
Usage:
    python scripts/data/prepare_mimir_domain.py --list-configs
    python scripts/data/prepare_mimir_domain.py --inspect --config github
...
from src.shard_audit.data.sanity_checks import (
    check_label_balance,
    check_word_counts,
...
def _drop_cross_label_duplicates(member_records: list, nonmember_records: list) -> tuple[list, int]:
    """Keep positive-source records and remove identical nonmember texts."""
    member_hashes = {record["text_hash"] for record in member_records}
    kept = [record for record in nonmember_records if record["text_hash"] not in member_hashes]
    return kept, len(nonmember_records) - len(kept)
...
def parse_args():
    ...
    p.add_argument("--allow-overlap", action="store_true",
                   help="Drop exact cross-label duplicates instead of failing preparation")
    return p.parse_args()
...
def main():
    ...
    if overlap_count > 0:
        ...
        if args.allow_overlap:
            # A duplicated text cannot validly carry both labels, and allowing
            # it through can place that same hash in both MIA splits.  Keep
            # the member record (the positive-source definition) and drop its
            # nonmember duplicate before sampling.
            nonmember_records, n_cross_label_removed = _drop_cross_label_duplicates(
                member_records, nonmember_records
            )
            logger.warning(
                "%s Dropped %d duplicate nonmember record(s) before splitting.",
                msg,
                n_cross_label_removed,
            )
        else:
            logger.error(msg + " Re-run with --allow-overlap to drop duplicate nonmembers.")
            sys.exit(1)
    else:
    ...
    manifest = {
        ...
        "cross_label_duplicates_removed": n_cross_label_removed,
