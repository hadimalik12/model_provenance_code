...
Like ``extract_logprob_scores_aug.py`` but uses domain-specific augmentations from
...
from src.shard_audit.text_augmentations_domains import augment_by_domain
...
def score_records_domain_aug(
    ...
    for idx, rec in enumerate(remaining):
        ...
        for view_txt in views:
            batch_lp = extract_token_logprobs(
                [view_txt], model, tokenizer, device, max_length=max_length
            )
            lp = batch_lp[0]
        ...
        orig_lp = extract_token_logprobs(
...
def main():
    ...
    try:
        model, tokenizer = load_model_and_tokenizer(
            args.model, device=device, dtype_str=args.dtype
        )
