"""MIA experiment: lomahony/eleuther-pythia12b-hh-dpo (Main + Control)

Target : lomahony/eleuther-pythia12b-hh-dpo
Parent : EleutherAI/pythia-12b
Dataset: MIMIR ArXiv (iamgroot42/mimir, config=github, split=ngram_13_0.2)
Primary: min_k_20_logprob

Run from the repo root:
    python scripts/experiments/run_target_lomahony_pythia_12b_hh_dpo.py
"""
...
# Configuration
# ------------------------------------------------------------------ #
TARGET_MODEL  = "lomahony/eleuther-pythia12b-hh-dpo"
TARGET_SLUG   = "lomahony_pythia_12b_hh_dpo"
...
PARENT_RESULTS  = "outputs/runs/mimir_arxiv_pythia_12b_mink/results.json"
...
def main() -> None:
    ...
    snapshot_download(TARGET_MODEL, max_workers=1)
    ...
    if not os.path.isfile(os.path.join(REPO_ROOT, PARENT_RESULTS)):
        ...
        print("Run scripts/experiments/run_parent_pythia_12b_arxiv.py first.")
        sys.exit(1)
