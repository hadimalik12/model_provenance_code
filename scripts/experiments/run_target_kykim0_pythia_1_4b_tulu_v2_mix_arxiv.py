"""MIA experiment: kykim0/pythia-1.4b-tulu-v2-mix (Main + Control)

Target : kykim0/pythia-1.4b-tulu-v2-mix
Parent : EleutherAI/pythia-1.4b
Dataset: MIMIR ArXiv (iamgroot42/mimir, config=github, split=ngram_13_0.2)
Primary: min_k_20_logprob

Run from the repo root:
    python scripts/experiments/run_target_kykim0_pythia_1_4b_tulu_v2_mix.py
"""
...
# Configuration
# ------------------------------------------------------------------ #
TARGET_MODEL  = "kykim0/pythia-1.4b-tulu-v2-mix"
TARGET_SLUG   = "kykim0_pythia_1_4b_tulu_v2_mix"
PARENT_MODEL  = "EleutherAI/pythia-1.4b"
PRIMARY_SCORE = "min_k_20_logprob"

# Data paths
TRAIN_FILE   = "data/processed/mimir_arxiv/train.jsonl"
TEST_FILE    = "data/processed/mimir_arxiv/test.jsonl"
CTRL_DIR     = "data/processed/mimir_arxiv_nonmember_control_seed0"
CTRL_TRAIN   = f"{CTRL_DIR}/train.jsonl"
CTRL_TEST    = f"{CTRL_DIR}/test.jsonl"

# Output paths (Main)
MAIN_SCORES_DIR = f"data/scores/mimir_arxiv_{TARGET_SLUG}"
MAIN_RUNS_DIR   = f"outputs/runs/mimir_arxiv_{TARGET_SLUG}"
PARENT_RESULTS  = "outputs/runs/mimir_arxiv_pythia_1_4_b_mink/results.json"

# Output paths (Control)
CTRL_SCORES_DIR = f"data/scores/mimir_arxiv_nonmember_control_seed0_{TARGET_SLUG}"
CTRL_RUNS_DIR   = f"outputs/runs/mimir_arxiv_nonmember_control_seed0_{TARGET_SLUG}"
...
def main() -> None:
    ...
    if not os.path.isfile(os.path.join(REPO_ROOT, PARENT_RESULTS)):
        # Try both 1_4b and 1_4_b slug if there's an inconsistency
        ALT_PARENT = "outputs/runs/mimir_arxiv_pythia_1_4b_mink/results.json"
        if os.path.isfile(os.path.join(REPO_ROOT, ALT_PARENT)):
            PARENT_RESULTS = ALT_PARENT
        else:
            print(f"ERROR: parent results not found: {PARENT_RESULTS}")
            print("Run scripts/experiments/run_parent_pythia_1_4b_arxiv.py first.")
            sys.exit(1)
