"""MIA experiment: EleutherAI/pythia-12b (Parent Baseline + Control)
...
Run from the repo root:
    python scripts/experiments/run_parent_pythia_12b_arxiv.py
"""
...
# Output paths (Main)
MAIN_SCORES_DIR = f"data/scores/mimir_arxiv_{SLUG}"
MAIN_RUNS_DIR   = f"outputs/runs/mimir_arxiv_{SLUG}"

# Output paths (Control)
CTRL_SCORES_DIR = f"data/scores/mimir_arxiv_nonmember_control_seed0_{SLUG}"
CTRL_RUNS_DIR   = f"outputs/runs/mimir_arxiv_nonmember_control_seed0_{SLUG}"
...
def main() -> None:
    ...
    snapshot_download(MODEL, max_workers=1)
    ...
    if not os.path.isfile(os.path.join(REPO_ROOT, TRAIN_FILE)):
        ...
        run([py, "scripts/data/prepare_mimir_arxiv.py", 
    ...
    if not os.path.isfile(os.path.join(REPO_ROOT, CTRL_TRAIN)):
        ...
        run([py, "scripts/data/prepare_mimir_arxiv_nonmember_control.py",
