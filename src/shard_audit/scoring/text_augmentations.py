...
def augment_code_snippet_4views(code_text: str, n_aug: int = 4) -> List[str]:
    ...
    """
    # A sentinel keeps malformed blank records tokenizable; ordinary records
    # retain their exact source for fallback views.
    source = code_text if isinstance(code_text, str) and code_text.strip() else "<empty>"
    lang = detect_language(source)
    views = []
    
    # View 1: Combined Augmentations (no original text)
    combined = _mutate_identifiers_lang_aware(source, lang)
    ...
    views.append(_mutate_identifiers_lang_aware(source, lang))
    
    # View 3: Safe String Quote Normalizer
    views.append(_normalize_string_quotes_safe(source))
    
    # View 4: Header & Whitespace Shift
    views.append(_shift_whitespace_lang_aware(source, lang))
    
    # Some syntactically valid snippets consist only of comments.  Keep every
    # view non-empty so downstream tokenizers always receive valid text.
    return [view if isinstance(view, str) and view.strip() else source for view in views[:n_aug]]
