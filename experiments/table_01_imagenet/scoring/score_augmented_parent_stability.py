"""Hybrid augmentation-based scoring: target encoder + parent head.

Combines Algorithm 2's augmentation stability with the white-box parent-head
projection. For each image x, generates K augmented views, extracts target
encoder features, projects through parent's 1000-class head, and computes
stability metrics on the resulting logit distributions.
"""
...
@torch.no_grad()
def aug_score_batch_hybrid(
    images, target_model, parent_head, augmenter, n_aug, device, dtype,
):
    """Score a batch: augment -> target encoder -> parent head -> stability."""
    target_model.eval()
    B = len(images)
    K = n_aug
    ...
    C = logits.shape[-1]  # 1000
    logits_bk = logits.view(B, K, C).float()
    log_p_bk  = log_p.view(B, K, C).float()
    probs_bk  = probs.view(B, K, C).float()

    # -- Stability metrics (identical to extract_image_scores_aug.py) --
    top1_p = probs_bk.max(dim=-1).values
    top1_std = top1_p.std(dim=-1, unbiased=False)

    logit_var_mean = logits_bk.var(dim=1, unbiased=False).mean(dim=-1)

    p_norm = F.normalize(probs_bk, p=2, dim=-1)
    cos_mat = torch.einsum("bkc,bjc->bkj", p_norm, p_norm)
    iu = torch.triu_indices(K, K, offset=1, device=cos_mat.device)
    cos_pairs = cos_mat[:, iu[0], iu[1]]
    cos_mean = cos_pairs.mean(dim=-1)

    p_bar = probs_bk.mean(dim=1, keepdim=True)
    log_p_bar = (p_bar + 1e-12).log()
    kl_per = (probs_bk * (log_p_bk - log_p_bar)).sum(dim=-1)
    kl_mean = kl_per.mean(dim=-1)
...
def parse_args():
    p = argparse.ArgumentParser(
        description="Hybrid augmentation scoring: target encoder + parent head.",
...
def main():
    ...
    logger.info("=== Hybrid Augmentation Scoring ===")
    ...
    score_keys = ("aug_neg_top1_std", "aug_neg_logit_var_mean",
                  "aug_cos_sim_mean", "aug_neg_kl_mean")
    ...
    def _score_split(records, split_name):
        ...
        for start in range(0, len(records), args.batch_size):
            ...
            stats = aug_score_batch_hybrid(
                imgs, target_model, parent_head, augmenter,
