...
@torch.no_grad()
def aug_score_batch_logprob(
    images, labels, target_model, parent_head, augmenter, n_aug, device, dtype,
):
    """Score a batch: augment -> target encoder -> parent head -> correct class log-probability."""
    target_model.eval()
    B = len(images)
    K = n_aug
    ...
    for b in range(B):
        out.append({
            "aug_logprob_mean": float(correct_logprob_mean[b].item()),
        })
    return out
...
def main():
    ...
    score_keys = ("aug_logprob_mean",)

    def _score_split(records, split_name, out_path):
        # Append each completed batch so a preempted Slurm allocation resumes
        # from record IDs instead of repeating the entire image workload.
        out = _load_jsonl(out_path) if os.path.exists(out_path) else []
        done_ids = {rec["id"] for rec in out}
        remaining = [rec for rec in records if rec["id"] not in done_ids]
        if not remaining:
            logger.info("  %s: all %d records already scored", split_name, len(records))
            return out
        ...
        with open(out_path, "a", encoding="utf-8") as handle:
            for start in range(0, len(remaining), args.batch_size):
                ...
                for rec, s in zip(batch, stats):
                    ...
                    out.append(score_rec)
                    handle.write(json.dumps(score_rec, ensure_ascii=False) + "\n")
                handle.flush()
                n_done = len(done_ids) + min(start + args.batch_size, len(remaining))
                if (start // args.batch_size + 1) % 10 == 0 or n_done == len(records):
