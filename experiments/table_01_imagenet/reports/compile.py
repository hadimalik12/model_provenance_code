...
for name in targets:
    path = f"{ARTIFACT_ROOT}/results/replicated_sweep_aug_logprob_{name}_vs_{ctrl}/results.json"
...
path_self = f"{ARTIFACT_ROOT}/results/replicated_sweep_aug_logprob_self_vs_{ctrl}/results.json"
...
path_deit = f"{ARTIFACT_ROOT}/results/replicated_sweep_aug_logprob_self_vs_deit_logprob/results.json"
...
os.makedirs(f"{ARTIFACT_ROOT}/reports", exist_ok=True)
with open(f"{ARTIFACT_ROOT}/reports/table_1.txt", "w") as f:
    f.write(output_str)
print(f"\nSaved compiled table to {ARTIFACT_ROOT}/reports/table_1.txt")
